  ...
  const inner = (
    <label className="block space-y-2">
            ...
            {DISPATCH_TABS.map((tab) => {
              ...
              return (
                ...
                >
                  {tab.label}
                  <span
                    className={`ml-1 tabular-nums ${active ? "text-blue-100" : "text-slate-500"}`}
                  >
                    ({count})
                  </span>
                </button>
      ...
      <input
        ...
        className="w-full rounded-lg border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 shadow-sm outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
      />
    </label>
  );
